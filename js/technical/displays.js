function prestigeButtonText(layer) {
	...
	if (tmp[layer].type == "normal")
		return `${player[layer].points.lt(1e3) ? (tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : "Reset for ") : ""}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource} ${tmp[layer].resetGain.lt(100) && player[layer].points.lt(1e3) ? `<br><br>Next at ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : tmp[layer].usePoints ? format(tmp[layer].nextAt, 4) : format(tmp[layer].nextAt))} ${tmp[layer].baseResource}` : ""}`
	if (tmp[layer].type == "static")
		return `${tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : "Reset for "}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource}<br><br>${player[layer].points.lt(30) ? (tmp[layer].baseAmount.gte(tmp[layer].nextAt) && (tmp[layer].canBuyMax !== undefined) && tmp[layer].canBuyMax ? "Next:" : "Req:") : ""} ${(tmp[layer].usePoints ? format(tmp[layer].baseAmount, 4) : formatWhole(tmp[layer].baseAmount))} / ${(tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAtDisp) : tmp[layer].usePoints ? format(tmp[layer].nextAtDisp, 4) : format(tmp[layer].nextAtDisp))} ${tmp[layer].baseResource}		
		`
